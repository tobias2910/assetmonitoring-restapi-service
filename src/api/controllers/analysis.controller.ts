...
export default class AnalysisController {
    ...
    public async obtainAggregatedTopAssets (req: Request, res: Response, next: NextFunction): Promise <void> {
        try {
            const query: AssetInformation =  {};
            ...
            const redisCacheKey = `aggregatedTop_analysis_query_${JSON.stringify(query)}_limit_${limit}`;
            let records = await Redis.getResult(redisCacheKey);

            if (!records) {
                records = await analysisService.getAggregatedTopAssets(query, limit);
                await Redis.saveResultWithTtl(redisCacheKey, records);    
            }
